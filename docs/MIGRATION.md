...
→ unveränderte Quelldatei hashen      SHA-256 → report.sourceSha256
...
| `euro_to_cents`         | Euro-Fließkomma → ganze Cent: `Math.round(euro * 100)` (kaufmännisch, halbe Cent weg von Null). Weicht das Ergebnis um mehr als 0,001 Cent vom Rechenwert ab (echter Präzisionsverlust jenseits der Float-Darstellung), erzeugt die Migration eine `warning` mit Pfad. `null`/fehlend bleibt `null`/fehlend. |
...
| `veraltet`               | `SUPERSEDED`                                       |
| anderer Wert             | `DRAFT` + `warning` (Originalwert im Issue-Detail) |
...
| 4.11 | `Stromzaehler`                                          | `Meter` (`heizkreis_id` per `ref_split`); `jahresstatus[jahr]` → `MeterBillingStatus` je Jahr mit vorhandener BillingPeriod; Jahre ohne BillingPeriod → `warning`, Eintrag wird trotzdem angelegt (`billingPeriodId` bleibt leer)                                                                                                                      |
...
`legacyUnmapped` (Record) im Zielobjekt ab — die `strictObject`-Schemas
des Zielformats führen dieses Feld explizit — und listet die
...
> Hinweis PR 04: `legacyUnmapped` ist im aktuellen Schema noch nicht an
> allen Entitäten vorhanden; es wird zusammen mit der Implementierung
> ergänzt (kleine, dokumentierte Schemaerweiterung — `strictObject`
> erzwingt, dass das nicht vergessen wird).
...
- `legacyUnmapped`-Feld an allen Zielentitäten ergänzen (Abschnitt 6).
