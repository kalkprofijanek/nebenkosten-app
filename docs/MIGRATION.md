...
| `euro_to_cents`         | Euro-Fließkomma → ganze Cent, kaufmännisch „halbe Cent weg von Null“ — auch für negative Beträge. Verbindliche Implementierung: `euroToCents()` in `packages/schema/src/migrations/euro-to-cents.ts` (normalisiert den Zwischenwert auf 15 signifikante Stellen gegen Float-Artefakte wie `1.005 * 100 === 100.499…`; `Math.round` allein wäre falsch, da es -0,5 Richtung +∞ rundet). Präzisionsverlust laut `euroToCentsLostPrecision()` erzeugt eine `warning` mit Pfad. `null`/fehlend bleibt `null`/fehlend. |
...
Das Feld `legacyUnmapped` ist seit PR 03 an allen persistierten Zielentitäten vorhanden (`legacyUnmappedSchema` in `entities/shared.ts`).
