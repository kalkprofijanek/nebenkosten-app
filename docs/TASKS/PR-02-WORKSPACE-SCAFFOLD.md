...
Status: in Umsetzung; Implementierungs-Start-Gate erfüllt
...
Claude getrennte Fachpakete implementieren, ohne Build-, Test-, Formatierungs- oder
...
1. das Claude-Review von PR 00 abgeschlossen ist,
...
Das Start-Gate ist erfüllt. Der lokale Branch `codex/pr02-workspace-scaffold` wurde vor der
Implementierung auf den freigegebenen `main`-Stand `9c0244c` rebasiert. Er wird erst nach
vollständiger lokaler Prüfung und den vorgesehenen Reviews gepusht.
...
Die privaten Pakete werden in PR 02 bewusst source-first über `src/index.ts` aufgelöst,
während ihr Build die isolierte Kompilierbarkeit nach `dist/` prüft. Eine veröffentlichte
Artefakt-API wird erst mit echten Paketverträgen festgelegt.
...
`pnpm run ci` ist bewusst ausgeschrieben, weil `pnpm ci` ein reservierter pnpm-Alias für
die Frozen-Lockfile-Installation ist und das gleichnamige Projektskript nicht startet.
...
`test:coverage` aggregiert beide Coverage-Gates, ohne Ergebnisse zu vermischen. Der
Vitest-Gate umfasst den vollständigen ausführbaren Web-Quellbaum einschließlich Bootstrap;
nur Tests und nicht ausführbare Styles sind ausgeschlossen.

`pnpm test` aggregiert Architektur-, Repository-, Unit-, Integrations- und Migrationstests.
Die einzelnen Skripte bleiben als getrennte CI-Jobs sichtbar. `privacy:scan` kombiniert den
Pfad-/Hash-Guard mit einem getesteten Inhalts-Scan auf konkrete E-Mail-Adressen, deutsche
IBANs, Zugangskennungen, private Schlüssel, Zugangswert-Zuweisungen und Straßenadressen.
`legacy/index.html` bleibt ausschließlich über seinen festen Hash geschützt. Binärdateien
werden standardmäßig blockiert, bis eine enge, hashgebundene Review-Ausnahme beschlossen ist.
Das generierte Lockfile wird separat auf reine Registry-Integritäten und interne
`workspace:*`-Links geprüft. `security:audit` führt mindestens
...
15. Claude-Review abwarten; kein Merge durch Codex.
